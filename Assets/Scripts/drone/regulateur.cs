...
public class regulateur {
	...
	private bool chooseControllerVecteur;
	...
	public regulateur(GameObject drone,moteur[] moteurDrone){
		...
		this.PIDxz = new PID (2.0F, 0.08F, 0.005F);
	...
	public void setControllerWithVecteur(bool drapeau){
		this.chooseControllerVecteur = drapeau;
	}
	...
	public void live(){
		...
		if (this.chooseControllerVecteur) {
			setMoveWithVecteur (new Vector3(0,3,3));
		} else {
			setMoveWithKey ();
		}
	...
	public void setMoveWithVecteur(Vector3 mouvement){
		...
		if (mouvement.x != 0 || mouvement.y != 0) {
			...
			if (mouvement.x > 0) {
				//directionX = lookForDirection.manageMovementAngle (CommandeDrone.Droite);
				directionX [0] = 30.0F;
				directionX [1] = 0.0F;
			} else {
				if (mouvement.x < 0) {
			...
			if (mouvement.z > 0) {
				//directionZ = lookForDirection.manageMovementAngle (CommandeDrone.Haut);
				directionZ [0] = 0.0F;
				directionZ [1] = -30.0F;
			} else {
				if (mouvement.z < 0) {
					//directionZ = lookForDirection.manageMovementAngle (CommandeDrone.Bas);
					directionZ [0] = 0.0F;
					directionZ [1] = -30.0F;
				}
			}
	...
	public void prepareMove(float angleX, float angleZ){
		...
		pidReturnY = PIDy.updatePID (getHauteur(), (float)drone.rigidbody.position.y, Time.deltaTime);
	...
	private void moveStabiliseAxe(float pidOz, float pidOx, float pidOy){
		...
		if (getCommande () != CommandeDrone.Monter && getCommande () != CommandeDrone.Descendre && getCommande() != CommandeDrone.Vecteur) {
			...
			setHauteur(drone.rigidbody.position.y);
		}
