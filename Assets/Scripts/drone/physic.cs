...
public class physic {
	...
	private float maxAcceleration(){
		// P = m * a
		float acceleration = (float) (forceDroneStabilise () * ConstantMaxAcceleration) / drone.rigidbody.mass;
		return acceleration;
	}
	...
	private float maxAccelerationDescente(){
		// P = m * a
		float acceleration = (float) (forceDroneStabilise () * ConstantMaxAccelerationDescente) / drone.rigidbody.mass;
		return acceleration;
	}
	...
	private float CurrentAcceleration(float force ){
		// a = F / m
		float accelerationCurrent = (force * 4) / drone.rigidbody.mass;
	...
	public float forceDroneStabilise(){
		...
		float forceDroneAppZ = (float) drone.rigidbody.mass * Physics.gravity.magnitude *  (1+ (1 - Mathf.Cos(drone.transform.eulerAngles.z * Mathf.Deg2Rad))) ;
		float forceDroneAppX = (float) drone.rigidbody.mass * Physics.gravity.magnitude *  (1+ (1 - Mathf.Cos(drone.transform.eulerAngles.x * Mathf.Deg2Rad))) ;
	...
	public float accelerationToForce(float acceleration){
		// F = a * m
		float force = acceleration * drone.rigidbody.mass;
		return force;
	}
